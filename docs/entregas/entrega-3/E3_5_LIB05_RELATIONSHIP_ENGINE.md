...
Para tipos direcionados: `UniqueConstraint(source_coid, target_coid,
relationship_type)` — tripla exata.
...
Isso faz `(A,B)` e `(B,A)` colidirem na mesma `UniqueConstraint`
(`TY3`, testado nos dois sentidos). A garantia **não depende
exclusivamente do caller** — é a mesma constraint de banco que também
protege tipos direcionados; a normalização apenas faz o caso simétrico
convergir para a mesma chave.
...
75 testes novos (71 unitários + 4 de integração):
...
- **Integração contra PostgreSQL real**
  (`test_relationship_integration.py`, 4): `PG1`-`PG5` — round-trip,
  FK, duplicidade, rollback, e **`PG5` — concorrência real de
  criação duplicada** (duas threads, duas sessões/conexões distintas,
  `threading.Barrier`, executado 4+ vezes, sempre exatamente 1
  commit).

**100% de cobertura de linha em todo `app/cognitive/`** (315 testes
unitários).

## PostgreSQL Result

4/4 testes de integração passando, incluindo `PG5` (concorrência
real) — nunca simulada apenas em SQLite.
...
100% em `app/cognitive/` (315 testes unitários).
...
770 passed, 18 skipped (sem `.env` local — mesmo padrão gracioso de
sempre), 97,54% (acima do anterior, 97,36%). Revalidado
explicitamente: COID imutável, CLID imutável após atribuição, lineage
append-only, `CURRENT` uniqueness por CLID (índice de E3.4.1 intacto
após a migração de `relationships`), `derive()`/`revise()`,
`TransformationRecord` append-only, cadeia de migrações completa,
7 erros mypy pré-existentes (não aumentaram). Nenhuma regressão.
