"""
Conftest de `tests/integration/connections/` (`E7.4-1`).

Mesma proteção — e mesma causa **medida** — de
`tests/integration/orchestration/conftest.py`: provas de round trip de
migration executam Alembic dentro do processo do pytest, e
...
que o encontram sujo. `autouse` para que a garantia não dependa de
alguém lembrar de aplicá-la ao próximo teste que rodar migração.

`alembic/env.py` **não** é alterado: o mecanismo já existe e tem prova de
não-vacuidade histórica.
"""
